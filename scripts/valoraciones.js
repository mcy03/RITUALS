...
function crearTablaResenas(data) {
    if (data.length > 0) {
        ...
        for (let index = 0; index < data.length; index++) {
            let estrellas = document.getElementById('td_resenas_puntuacion'+(index+1));

            console.log(parseInt(data[index].VALORACION));
...
async function insertResena(event) {
    event.preventDefault();
    const pedido = document.getElementById('pedido').value;
    console.log(await tieneResenas(pedido));
    if (await tieneResenas(pedido) > 0) {
        error('el pedido '+ pedido +' ya tiene reseña');
    }else{
        if (pedido == "undefined") {
            warning('Porfavor, seleccione un pedido al que aplicar la reseña');
        }else{
...
async function tieneResenas(PEDIDO_ID) {
    let formData = new FormData();
        formData.append('accion', 'count_reviews_command');
...
function mensajeSinResenas(puntuacion = ""){
    ...
    if (puntuacion == "") {
        p.innerHTML = "No hay reseñas registradas";
    }else{
...
const misResenas = document.getElementById('boton-gestion');

misResenas.addEventListener('click', mostrarMisResenas);

async function mostrarMisResenas() {
    let usuario_guardado = JSON.parse(localStorage.getItem('user'));
    let resenas = await misresenasApi(usuario_guardado[0].USUARIO_ID);
    console.log(resenas);
    if (resenas.length < 1 || resenas == false) {
        warning('no has publicado ninguna reseña');
    }else{
        crearTablaResenas(resenas, true);
    }
}


async function misresenasApi(usuario_id) {
    let formData = new FormData();
        formData.append('accion', 'get_my_reviews');
        formData.append('user', usuario_id);
...
const allResenas = document.getElementById('boton-todas');

allResenas.addEventListener('click', mostrarAllResenas);

async function mostrarAllResenas() {
    let resenas = await resenasApi();
}